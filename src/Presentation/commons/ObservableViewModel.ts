...
export default class ObservableViewModel implements ObservableContext {
  ...
  unsubscribeDataChanged(
    ...
    const currentDataChangedHandlerList =
      this._dataChangedHandler.get(fieldName);
  ...
  dispatchDataChanged(fieldName?: string): void {
    if (fieldName) {
      this.dispatchFieldChanged(fieldName);
    } else {
      this.dispatchAll();
    }
  }

  private dispatchFieldChanged(fieldName: string): void {
    const currentDataChangedHandlerList =
      this._dataChangedHandler.get(fieldName);
  ...
  private dispatchAll() {
    this._dataChangedHandler.forEach((value, key) =>
      this.dispatchFieldChanged(key)
    );
  }
}
